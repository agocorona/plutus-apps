...
import Database.Beam (Columnar, Database, DatabaseEntity, DatabaseSettings, Identity, SqlSelect, TableEntity,
                      aggregate_, all_, countAll_, delete, filter_, guard_, limit_, not_, nub_, select, val_)
...
getUtxoutFromRef ref =
    ask >>= \db -> (<$>) (fromTxOut =<<) $ queryOne $ queryKeyValue db utxoOutRefRows _utxoRowOutRef _utxoRowTxOut ref
