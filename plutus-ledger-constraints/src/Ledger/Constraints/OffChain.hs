...
processConstraintFun = \case
    MustSpendScriptOutputWithMatchingDatumAndValue vh datumPred valuePred red -> do
        ...
        opts <- filter (matches . snd) <$> traverse (\(ref, txo) -> (ref,) <$> resolveTxOut txo) (Map.toList slTxOutputs)
        case opts of
...
resolveTxOut Tx.ScriptChainIndexTxOut { Tx._ciTxOutValidator, Tx._ciTxOutDatum, Tx._ciTxOutValue } = do
